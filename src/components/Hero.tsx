  ...
  return (
    ...
    >
      ...
      <g fill="#0b0805">
        ...
        <line x1="508" y1="78" x2="462" y2="92" stroke="#0b0805" strokeWidth="5" />
        <line x1="508" y1="78" x2="556" y2="88" stroke="#0b0805" strokeWidth="5" />
        <line x1="466" y1="92" x2="466" y2="118" stroke="#0b0805" strokeWidth="2" />
      ...
      <rect x="0" y="146" width="1200" height="2" fill="rgba(201,150,46,0.35)" />
    </svg>
  )
}
...
function BigBen() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      minuteRef.current?.setAttribute('transform', `rotate(${minutes * 6} 50 92)`)
  ...
  return (
    <div className="big-ben" aria-hidden="true">
      <svg viewBox="0 0 100 300">
        ...
        <circle cx="50" cy="92" r="22" fill="none" stroke="#c9962e" strokeWidth="3" />
        <circle cx="50" cy="92" r="16.5" fill="none" stroke="rgba(90,60,20,0.45)" strokeWidth="1" />
        {ticks.map((angle) => (
          <line
            key={angle}
            x1="50"
...
function TrainWheel({ cx, cy, r }: { cx: number; cy: number; r: number }) {
  return (
    <g>
      <circle cx={cx} cy={cy} r={r} fill="#241a0e" stroke="rgba(201,150,46,0.4)" strokeWidth="1.5" />
      <g className="train-wheel">
        <line x1={cx - r + 2} y1={cy} x2={cx + r - 2} y2={cy} stroke="rgba(201,150,46,0.4)" strokeWidth="1.4" />
        <line x1={cx} y1={cy - r + 2} x2={cx} y2={cy + r - 2} stroke="rgba(201,150,46,0.4)" strokeWidth="1.4" />
      </g>
  ...
  return (
    <div className="hero-train" aria-hidden="true">
      <svg width="560" height="60" viewBox="0 0 560 60">
        ...
        <rect x="6" y="22" width="120" height="26" rx="4" fill={body} stroke={rim} strokeWidth="1" />
        ...
        <rect x="140" y="22" width="120" height="26" rx="4" fill={body} stroke={rim} strokeWidth="1" />
        <g fill={win} opacity="0.8">
        ...
        {/* tênder */}
        <rect x="272" y="26" width="60" height="22" rx="3" fill={body} stroke={rim} strokeWidth="1" />
        ...
        <rect x="340" y="12" width="44" height="36" rx="3" fill={body} stroke={rim} strokeWidth="1" />
        <rect x="350" y="18" width="14" height="12" rx="1" fill={win} opacity="0.85" />
        <rect x="384" y="22" width="130" height="24" rx="11" fill={body} stroke={rim} strokeWidth="1" />
        <circle cx="514" cy="34" r="12" fill={body} stroke={rim} strokeWidth="1" />
        ...
        <polygon points="526,24 560,16 560,44 526,36" fill="#ffb84d" opacity="0.12" />
  ...
  useEffect(() => {
    ...
    intro
      ...
      .add(
        ...
        { opacity: [0, 1], translateY: [16, 0], duration: 700, delay: stagger(120) },
        '-=200',
      )
    ...
    for (const [selector, turn, duration] of spinConfig) {
      ...
      if (!el) continue
      animations.push(animate(el, { rotate: turn, duration, ease: 'linear', loop: true }))
    }
  ...
  return (
    <header className="hero" id="inicio" ref={rootRef}>
      ...
      <div className="hero-gears" aria-hidden="true">
        <Gear className="hero-gear gear-big" size={380} teeth={18} spokes={6} color="#6f5223" />
        <Gear className="hero-gear gear-m1" size={210} teeth={14} spokes={6} color="#8a6a2f" />
        <Gear className="hero-gear gear-m2" size={130} teeth={9} spokes={4} color="#b06a3b" />
      </div>
      ...
      <div className="steam" aria-hidden="true">
        {CHIMNEYS.map((left) => (
          <span key={left} className="steam-puff" style={{ left: `${left}%` }} />
        ))}
      </div>
      ...
      <div className="hero-content">
        ...
        <div className="hero-contacts">
          <a href={`mailto:${profile.email}`} className="brass-chip">
            ✉ {profile.email}
          </a>
        </div>
        <a
